...
payload_filters = {
        ...
        "sortBy": ["severity:asc", "alertCount:asc"],
        "size": 100,
        "nextPageToken": "",
        "searchText": "",
        "detailed": True
...
##################################################################################################################
print(f"[*] start_time: {start_2} ({start})")
print(f"[*] end_time: {end_2} ({end})")
...
def process_alert_data(raw_json, policy_finding_map):
    ...
    for item in raw_json:
        ...
        if sev in alert_groups:
            if p_id not in alert_groups[sev]:
                alert_groups[sev][p_id] = {'policy': item['policy'], 'resources': [], 'count': 0} # 정책별 그룹 생성
            alert_groups[sev][p_id]['resources'].append(item)
            alert_groups[sev][p_id]['count'] += 1
...
def wordCreate(alert_groups):
    ...
    # 템플릿에서 사용하기 편하도록 딕셔너리를 리스트 형태로 변환하는 내부 함수
    def format_severity_data(severity_dict, start_index=1):
        result = []
        current_index = start_index
        for policy_id, data in severity_dict.items():
            result.append({
                'policy': data['policy'],
                'resources': data['resources'],
                'count': data['count'],
                'cloudType': data['resources'][0]['resource']['cloudType'] if data['resources'] else 'N/A',
                'index': current_index
            })
            current_index += 1
        return result, current_index

    idx = 1 # 보고서에 표시될 순번(No.)
    alerts_critical, idx = format_severity_data(alert_groups['critical'], idx)
    alerts_high, idx = format_severity_data(alert_groups['high'], idx)
    alerts_medium, idx = format_severity_data(alert_groups['medium'], idx)
    alerts_low, idx = format_severity_data(alert_groups['low'], idx)
    alerts_info, idx = format_severity_data(alert_groups['info'], idx)
    
    # 워드 템플릿의 {{ 변수명 }} 자리에 들어갈 데이터 매핑
    context = {
        'alerts': alerts_critical + alerts_high + alerts_medium + alerts_low + alerts_info,
        ...
        'critical_count': sum(len(v['resources']) for v in alert_groups['critical'].values()),
        'high_count': sum(len(v['resources']) for v in alert_groups['high'].values()),
        'medium_count': sum(len(v['resources']) for v in alert_groups['medium'].values()),
        'low_count': sum(len(v['resources']) for v in alert_groups['low'].values()),
        'info_count': sum(len(v['resources']) for v in alert_groups['info'].values()),
